...
$query0 = "SELECT COUNT(*) FROM $articles JOIN $categories ON $articles.category_id = $categories.cat_id AND cat_seo_name != ? AND cat_seo_name != ? AND $articles.publish = ?";

$result0 = $link->GetRow($query0, ['slusalica', 'preuzimanja', 1]);
$total = ($result0['COUNT(*)']);
...
if (isset($_GET['content2']) AND $_GET['content2'] === 'like') {
	...
	$query = "SELECT * FROM $articles JOIN $users ON $articles.author_id = $users.userid JOIN $categories ON $articles.category_id = $categories.cat_id AND cat_seo_name != ? AND cat_seo_name != ? AND $articles.publish = ? ORDER BY like_article DESC LIMIT $start, $limit";
} else if (isset($_GET['content2']) AND $_GET['content2'] === 'dislike') {
	
	$order_notice = $c['order_dislike_ar'];
	$query = "SELECT * FROM $articles JOIN $users ON $articles.author_id = $users.userid JOIN $categories ON $articles.category_id = $categories.cat_id AND cat_seo_name != ? AND cat_seo_name != ? AND $articles.publish = ? ORDER BY dislike_article DESC LIMIT $start, $limit";
} else if (isset($_GET['content2']) AND $_GET['content2'] === 'old') {
	
	$order_notice = $c['order_old_ar'];
	$query = "SELECT * FROM $articles JOIN $users ON $articles.author_id = $users.userid JOIN $categories ON $articles.category_id = $categories.cat_id AND cat_seo_name != ? AND cat_seo_name != ? AND $articles.publish = ? ORDER BY article_id ASC LIMIT $start, $limit";
} else {
	
	$order_notice = $c['order_new_ar'];
	$query = "SELECT * FROM $articles JOIN $users ON $articles.author_id = $users.userid JOIN $categories ON $articles.category_id = $categories.cat_id AND cat_seo_name != ? AND cat_seo_name != ? AND $articles.publish = ? ORDER BY article_id DESC LIMIT $start, $limit";
}

$result = $link->GetRows($query, ['slusalica', 'preuzimanja', 1]);
...
if (!empty($result)) {
	...
	$output = "
		...
		<h2 class='head'>$c[all_articles]</h2>
